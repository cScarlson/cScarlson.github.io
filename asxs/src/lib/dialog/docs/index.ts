
export { default as docs } from './docs.md?raw';
export { default as setuphtml } from './setup.html?raw';
export { default as setupts } from './setup.ts?raw';
export { default as normal } from './normal.html?raw';
export { default as queued } from './queued.html?raw';
export { default as script } from './queued.js?raw';
export { default as backdrop } from './backdrop.html?raw';
export { default as fullscreen } from './fullscreen.html?raw';
export { default as conventional } from './conventional.html?raw';
export { init } from './queued.setup';
